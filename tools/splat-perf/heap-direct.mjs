...
const child=spawn(chromium.executablePath(),[...GPU_LAUNCH_ARGS,"--headless=new","--remote-debugging-port=0",`--user-data-dir=${profile}`,"about:blank"],{stdio:"ignore"});
...
try {
 ...
 await call("Page.navigate",{url:"http://127.0.0.1:8099/tools/splat-perf/loading-harness.html?production=candidate&mode=static&sse=8&frames=30&warmup=60&width=960&height=540"});
 for(let i=0;i<2400;i++){const ready=await evaluate("!!window.__benchResult || window.__benchError");if(ready){if(typeof ready==="string"){throw new Error(ready);}break;}await sleep(100);if(i===2399){throw new Error("Load timeout");}}
 const rows=[];
