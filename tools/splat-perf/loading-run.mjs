    ...
    try {
      ...
      if (run.lifecycle === "1") {
        ...
        for(let cycle=0;cycle<Number(run.cycles||4);cycle++) {
          ...
          await cdp.send("HeapProfiler.collectGarbage");
          result.lifecycle.push({cycle,stage:"removed",workerHeaps:await workerHeaps(),worker:await cache(),heap:await cdp.send("Runtime.getHeapUsage"),state:await page.evaluate(()=>({gl:window.__telemetry.gl,loading:window.__loadingStats,queue:window.__loadingQueueState()}))});
