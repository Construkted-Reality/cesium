...
try {
 ...
 await page.goto('http://localhost:8099/tools/splat-perf/loading-harness.html?tileset=/splat-data/oracle-run/bike-newdefault/tileset.json&tileCache='+Math.round(+budget*1048576)+'&sse=4&warmup=120&frames=30&mode=static&capture=1');
 ...
 await page.waitForFunction(()=>{const t=__round2Tilesets[0],p=t.gaussianSplatPrimitive;return t.tilesLoaded&&t.memoryAdjustedScreenSpaceError<=4.0001&&t.statistics.numberOfTilesWithContentReady===72&&p?._numSplats===705883&&!p._pendingSnapshot&&t._selectedTiles.every(tile=>p._selectedTileSet.has(tile));},null,{timeout:90000});
