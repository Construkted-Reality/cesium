...
const G=Cesium.GaussianSplatPrimitive,S=Cesium.GaussianSplatSorter;
const generate=G.generateSplatTexture,sort=S.radixSortIndexes;
const prefetched=new Map();
G.generateSplatTexture=function(p,f,s){
 if(s?.state==='BUILDING'&&!prefetched.has(p._sorterPositionsKey)){
  const positions=p._prepareSorterPositions(p._splatDataGeneration,s.positions);
  const modelView=Float32Array.from(Cesium.Matrix4.multiply(f.camera.viewMatrix,p._rootTransform,new Cesium.Matrix4()));
  const key=p._sorterPositionsKey;
  const promise=sort.call(S,{primitive:{positions,modelView,count:s.numSplats},sortType:'Index',positionsKey:key});
  if(promise){promise.catch(()=>{});prefetched.set(key,{promise,modelView});}else p._invalidateSorterPositions();
 }
 return generate.apply(this,arguments);
};
S.radixSortIndexes=function(params){const p=prefetched.get(params.positionsKey);if(p){prefetched.delete(params.positionsKey);if(p.modelView.every((v,i)=>v===params.primitive.modelView[i]))return p.promise;}return sort.apply(this,arguments);};
