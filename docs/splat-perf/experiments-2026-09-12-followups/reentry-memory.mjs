...
P.buildGSplatDrawCommand=function(p){p._updateMemoryStatistics();const r=build.apply(this,arguments);p._updateMemoryStatistics();return r;};
 ...
 const processes=await browserSession.send('SystemInfo.getProcessInfo');stage.processMemory=[];
 for(const process of processes.processInfo){try{const rollup=readFileSync('/proc/'+process.id+'/smaps_rollup','utf8');const memory=Object.fromEntries([...rollup.matchAll(/^(Rss|Pss|Private_Dirty):\s+(\d+) kB/gm)].map(x=>[x[1],Number(x[2])]));stage.processMemory.push({type:process.type,...memory});}catch{}}
 ...
 await page.evaluate(label=>{__scene.primitives.removeAll();if(label.includes('release')){const G=Cesium.GaussianSplatTextureGenerator;const t=G._textureTaskProcessor;if(t&&t._activeTasks===0){t.destroy();G._textureTaskProcessor=undefined;G._taskProcessorReady=false;}}},label);
 result.workerAfterDisposal=await page.evaluate(()=>({alive:!!Cesium.GaussianSplatTextureGenerator._textureTaskProcessor,references:Cesium.GaussianSplatTextureGenerator._referenceCount}));
 ...
 const browserSession=await browser.newBrowserCDPSession();const processes=await browserSession.send('SystemInfo.getProcessInfo');result.disposedMemory=[];
 for(const process of processes.processInfo){try{const rollup=readFileSync('/proc/'+process.id+'/smaps_rollup','utf8');const memory=Object.fromEntries([...rollup.matchAll(/^(Rss|Pss|Private_Dirty):\s+(\d+) kB/gm)].map(x=>[x[1],Number(x[2])]));result.disposedMemory.push({type:process.type,...memory});}catch{}}
 await browserSession.detach();

 await page.evaluate(async()=>{__scene.requestRenderMode=true;__scene.maximumRenderTimeChange=Infinity;window.__reloaded=await Cesium.Cesium3DTileset.fromUrl('/splat-data/oracle-run/geo-newdefault/tileset.json',{maximumScreenSpaceError:4,cacheBytes:1073741824});__scene.primitives.add(__reloaded);__scene.requestRender();});
 await page.waitForFunction(()=>__reloaded.gaussianSplatPrimitive?._numSplats===2087136&&!__reloaded.gaussianSplatPrimitive._pendingSnapshot&&__reloaded.tilesLoaded,null,{timeout:60000});
 result.reentry=await page.evaluate(()=>({splats:__reloaded.gaussianSplatPrimitive._numSplats,workerAlive:!!Cesium.GaussianSplatTextureGenerator._textureTaskProcessor,references:Cesium.GaussianSplatTextureGenerator._referenceCount}));
