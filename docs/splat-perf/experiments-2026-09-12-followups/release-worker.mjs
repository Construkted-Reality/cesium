...
const generateOriginal=Cesium.GaussianSplatTextureGenerator.generateFromAttributes;
Cesium.GaussianSplatTextureGenerator.generateFromAttributes=function(){const result=generateOriginal.apply(this,arguments);if(result)result.then(()=>{const G=Cesium.GaussianSplatTextureGenerator;const t=G._textureTaskProcessor;if(t?._activeTasks===0){t.destroy();G._textureTaskProcessor=undefined;G._taskProcessorReady=false;}});return result;};
